(function () {
    ...
    return (event.ticket_types || [])
      .filter(
        (tier) =>
          ...
          tier.status !== "paused" &&
          Number(tier.quantity_available ?? tier.remaining ?? 0) > 0,
      )
      ...
      event.available_inventory ??
        tiers.reduce((sum, tier) => {
          const tierAvailable =
            ...
            Math.max(
              0,
              Number(tier.quantity_total || 0) -
                Number(tier.quantity_sold || 0) -
                Number(tier.quantity_reserved || 0),
            );
  ...
  function salesStatus(event) {
    ...
    if (apiState === "ended") {
      return {
        key: "ended",
        label: tr("events.event_ended", "Event Ended"),
        priceLabel: tr("events.sales_closed", "Sales Closed"),
        canBuy: false,
        labelKey: "events.event_ended",
        priceKey: "events.sales_closed",
      };
    }
    if (apiState === "sold_out") {
      return {
        key: "sold_out",
        label: tr("events.sold_out", "Sold Out"),
        priceLabel: tr("events.sold_out", "Sold Out"),
        canBuy: false,
        labelKey: "events.sold_out",
        priceKey: "events.sold_out",
      };
    }
    if (apiState === "live") {
      return {
        key: "live",
        label: tr("events.live", "Live"),
        priceLabel: "",
        canBuy: true,
        labelKey: "events.live",
      };
    }
    if (apiState === "upcoming") {
      return {
        key: "upcoming",
        label: tr("events.upcoming", "Upcoming"),
        priceLabel: "",
        canBuy: true,
    ...
    if (
      ...
      (event?.price_from !== undefined || event?.base_price !== undefined)
    ) {
  ...
  function lowestAvailablePrice(event) {
    ...
    const tier =
      availableTicketTypes(event)[0] ||
      (event.ticket_types || []).sort((a, b) => Number(a.price || 0) - Number(b.price || 0))[0];
    return tier
      ? { amount: tier.price, currency: tier.currency || event.currency }
      : { amount: event.base_price ?? 0, currency: event.currency || "USD" };
  }
  ...
  function renderEventCard(event, index) {
    ...
    const cat =
      status.key === "ended" || status.key === "sold_out" || status.key === "live"
    ...
    const imageAlt =
      [event.title, event.venue_name, event.city].filter(Boolean).join(" at ") ||
      tr("events.event", "Event");
