(function () {
  ...
  function eventCard(event) {
    const image =
      event.banner_image_url || event.images?.[0]?.url || "../assets/img/concert-hero.svg";
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      window.__scannerDashboardEvents =
        data.assigned_events || (data.assigned_event ? [data.assigned_event] : []);
      renderEvents(window.__scannerDashboardEvents);
