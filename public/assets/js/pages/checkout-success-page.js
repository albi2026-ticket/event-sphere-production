    ...
    const poll = async () => {
      try {
        ...
        if (data.payment_status === "paid" || data.status === "paid") {
          if (statusEl)
            statusEl.textContent = `Order ${data.order_number || ""} confirmed. Your tickets are ready.`;
          if (link) link.href = "/dashboard";
          const notificationKey = `order-confirmed-${data.id || orderId}`;
          const exists = window.EventSphereNotifications?.list?.().some(
            (item) => item.id === notificationKey,
          );
          if (!exists) {
        ...
        if (++attempts < 20) setTimeout(poll, 2000);
          ...
          statusEl.textContent = "Payment is still processing. Check your dashboard shortly.";
      } catch (err) {
