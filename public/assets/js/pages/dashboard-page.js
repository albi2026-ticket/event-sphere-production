(function () {
  ...
  function ticketDisplayStatus(ticket) {
    if (isEndedEvent(ticket.event) && ticket.status === "valid")
      return tr("events.event_ended", "Event Ended");
  ...
  function ticketParams() {
    ...
    const params = {
      per_page: state.ticketView === "ended" ? 100 : 6,
  ...
  function renderGreeting() {
    ...
    document
      .querySelector("[data-dashboard-greeting]")
      ?.replaceChildren(document.createTextNode(`Hi, ${name}`));
    ...
    document
      .querySelector("[data-dashboard-subtitle]")
      ?.replaceChildren(document.createTextNode(subtitle));
  }
  ...
  function renderKpis() {
    ...
    if (state.loading.summary) {
      kpiRow.innerHTML = Array.from({ length: 4 })
        .map(
          () =>
  ...
  function renderUpcomingEvents() {
    ...
    if (state.loading.upcoming) {
      if (el)
        el.innerHTML = loadingState(
          tr("dashboard.loading_upcoming_events", "Loading upcoming events..."),
        );
      if (detail)
        detail.innerHTML = loadingState(
          tr("dashboard.loading_upcoming_events", "Loading upcoming events..."),
        );
      return;
    }
    ...
    const upcomingTickets = state.upcomingTickets.length
      ? state.upcomingTickets
      : state.tickets.filter((ticket) => isUpcomingEvent(ticket.event));
    const ticketByEvent = new Map(
      upcomingTickets.map((ticket) => [String(ticket.event?.id), ticket]),
    );
  ...
  function renderTickets() {
    ...
    if (state.loading.tickets) {
      el.innerHTML =
        ...
        `<div class="col-12">${loadingState(tr("loading.loading_tickets", "Loading tickets..."))}</div>`;
  ...
  function renderOrders() {
    ...
    if (state.loading.orders) {
      body.innerHTML =
        window.EventSphereSkeleton?.tableRows?.(7, 5) ||
        `<tr><td colspan="7">${loadingState(tr("orders.loading_orders", "Loading orders..."))}</td></tr>`;
      if (pager) pager.innerHTML = "";
      return;
    }
  ...
  function renderFavoriteCard(favorite) {
    ...
    const stateBadge =
      event.event_state?.label ||
      (event.is_sold_out
        ? tr("events.sold_out", "Sold Out")
        : isUpcomingEvent(event)
          ? tr("events.upcoming", "Upcoming")
  ...
  function renderFavorites() {
    ...
    if (state.loading.favorites) {
      el.innerHTML =
        window.EventSphereSkeleton?.eventCards?.(3, "col-md-6 col-xl-4") ||
        `<div class="col-12">${loadingState(tr("dashboard.loading_favorites", "Loading favorites..."))}</div>`;
      if (pager) pager.innerHTML = "";
      return;
    }
  ...
  function renderActivity() {
      ...
      ...(state.summary?.recent?.orders || []).map((order) => ({
        ...
        title:
          order.payment_status === "paid"
            ? `${tr("dashboard.order_confirmation", "Order confirmation")}: ${order.order_number}`
            : `${tr("tickets.order_label", "Order")} ${order.order_number}`,
      ...
      ...state.tickets.slice(0, 3).map((ticket) => ({
        ...
        title:
          ticket.status === "checked_in"
            ? `${tr("dashboard.ticket_checked_in", "Ticket Checked In")}: ${ticket.event?.title || ticket.ticket_code}`
            : `${tr("dashboard.ticket_purchased", "Ticket Purchased")}: ${ticket.event?.title || ticket.ticket_code}`,
    ...
    `,
        ...
        .join("") ||
      `<li>${emptyState("bi-clock-history", tr("dashboard.no_recent_activity", "No recent activity"), tr("dashboard.activity_appear", "Your purchases, tickets, and favorites will show here."))}</li>`;
  }

  function attendedTickets() {
    return state.historyTickets.filter(
      (ticket) => ticket.status === "checked_in" || ticket.checked_in_at,
    );
  }
  ...
  function renderAttendanceHistory() {
    ...
    const mostRecent = [...attended].sort(
      (a, b) =>
        new Date(b.checked_in_at || b.event?.starts_at || 0) -
        new Date(a.checked_in_at || a.event?.starts_at || 0),
    )[0];
    ...
    if (state.loading.history) {
      body.innerHTML =
        window.EventSphereSkeleton?.tableRows?.(4, 4) ||
        `<tr><td colspan="4">${loadingState(tr("dashboard.loading_attendance_history", "Loading attendance history..."))}</td></tr>`;
      return;
    }
        ...
        ? [
            {
              ...
              detail:
                '<button class="btn btn-glass btn-sm mt-2" type="button" data-profile-resend-verification>Resend Verification Email</button>',
            },
          ]
        : []),
    ];
  ...
  function showProfileError(message) {
    ...
    if (el)
      el.innerHTML = message
        ? `<div class="alert border-pro text-danger dashboard-note">${escape(message)}</div>`
        : "";
  }
  ...
  async function showTicketDetails(ticketId) {
    ...
    detailModal(
      tr("tickets.ticket_details", "Ticket Details"),
      `
  ...
  async function showQr(ticketId) {
    detailModal(
      tr("tickets.ticket_qr_code", "Ticket QR code"),
      `<div class="dashboard-qr-wrap">${loadingState(tr("tickets.loading_qr_code", "Loading QR code..."))}</div>`,
    );
    try {
  ...
  async function showOrderDetails(orderId) {
    ...
    detailModal(
      tr("orders.order_details", "Order Details"),
      `
      <div class="dashboard-detail-grid">
  ...
  function bindActions() {
    document.addEventListener("click", async (event) => {
      ...
      if (ticketDownload) {
        event.preventDefault();
        try {
          await ticketsApi().downloadTicket(
            ticketDownload.dataset.ticketDownload,
            ticketDownload.dataset.ticketCode,
          );
        } catch (err) {
          window.tkToast?.(
      ...
      if (receipt) {
        event.preventDefault();
        try {
          await ordersApi().downloadReceipt(
            receipt.dataset.orderReceipt,
            receipt.dataset.orderNumber,
          );
        } catch (err) {
          window.tkToast?.(
