(function () {
  ...
  async function loadPerformance() {
    ...
    try {
      const { data } = await api().fetch(
        "/organizer/events/performance?sort=-revenue&per_page=100",
      );
  ...
  async function loadRevenue() {
    ...
    try {
      const { data } = await api().fetch(
        `/organizer/analytics/revenue?group_by=${encodeURIComponent(state.groupBy)}`,
      );
      state.revenue = data;
  ...
  function renderKpis() {
    ...
    if (state.loading.summary) {
      kpiRow.innerHTML = Array.from({ length: 6 })
        .map(
          () =>
            '<div class="col-md-4 col-xl-2"><div class="kpi"><div class="skel dashboard-skel-line"></div><div class="skel dashboard-skel-value"></div></div></div>',
        )
        .join("");
      return;
    }
    const cards = state.summary?.cards || {};
    const liveNow = state.events.filter(
      (event) => organizerEventState(event).key === "live",
    ).length;
    kpiRow.innerHTML = `
    ...
    if (subtitle)
      subtitle.textContent = `${cards.upcoming_events_count ?? 0} upcoming events · ${cards.sold_out_ticket_types_count ?? 0} sold-out ticket tiers`;
    renderOverview();
    renderRevenueKpis();
    renderSettings();
  }
  ...
  function renderRecentActivity() {
    ...
    const orders = rows(state.summary?.recent_orders).map((order) => ({
      ...
      title: tr("notifications.ticket_purchased_title", "New ticket purchased"),
    ...
    const checkIns = rows(state.summary?.recent_attendees)
      ...
      .map((ticket) => ({
        ...
        title: tr("notifications.attendee_checked_in_title", "New attendee checked in"),
    ...
    const eventActivity = state.events.flatMap((event) => {
      ...
      if (event.status === "published") {
        activity.push({
          icon: "bi-broadcast",
          title: tr("notifications.event_published_title", "Event published"),
          detail: event.title,
      ...
      if (organizerEventState(event).key === "sold_out") {
        activity.push({
          ...
          title: tr("notifications.event_sold_out_title", "Event sold out"),
          detail: event.title,
    ...
    `,
      ...
      emptyBlock(
        ...
        "Purchases, check-ins, event updates, and sold-out events will appear here.",
      );
  }
  ...
  function renderUpcomingEvents() {
    ...
    if (state.loading.events) {
      wrap.innerHTML =
        window.EventSphereSkeleton?.eventCards?.(3, "col-md-6 col-xl-4") ||
        loadingBlock("Loading events...");
      return;
    }
    ...
    wrap.innerHTML =
      ...
      emptyBlock(
        "bi-calendar-week",
        "No upcoming events",
        "Published future events and live events will appear here.",
      );
  }
  ...
  function renderRevenueKpis() {
    ...
    if (state.loading.revenue || state.loading.ticketAnalytics) {
      row.innerHTML = Array.from({ length: 4 })
        .map(
          () =>
            '<div class="col-md-3"><div class="kpi"><div class="skel dashboard-skel-line"></div><div class="skel dashboard-skel-value"></div></div></div>',
        )
        .join("");
      return;
    }
    ...
    const today = state.salesDayTrends.find(
      (item) => String(item.period).slice(0, 10) === todayKey,
    );
    row.innerHTML = `
      ...
      <div class="col-md-3"><div class="kpi"><div class="label">${esc(tr("events.this_month", "This Month"))}</div><div class="value">${u().formatMoney(
        revenueFor((date) => date >= startOfMonth),
        "USD",
      )}</div><div class="delta">Month to date</div></div></div>
      <div class="col-md-3"><div class="kpi"><div class="label">${esc(tr("events.this_week", "This Week"))}</div><div class="value">${u().formatMoney(
  ...
  function renderRevenue() {
    ...
    `,
      ...
      emptyRow(
        4,
        "bi-cash-stack",
        tr("organizer.no_revenue_yet", "No revenue yet."),
        tr("organizer.no_revenue_copy", "Paid orders will appear here as guests buy tickets."),
      );
  }
  ...
  function renderRevenueChart() {
    ...
    if (state.loading.revenue || state.errors.revenue) {
      if (empty)
        empty.innerHTML = state.errors.revenue
          ? emptyBlock("bi-exclamation-triangle", "Revenue chart unavailable", state.errors.revenue)
          : "";
      return;
    }
    ...
    if (!labels.length) {
      if (empty)
        empty.innerHTML = emptyBlock(
          "bi-graph-up",
          tr("organizer.no_revenue_trend", "No revenue trend yet."),
          tr("organizer.no_revenue_trend_copy", "Revenue trends will appear after paid orders."),
        );
      return;
    }
  ...
  function renderRevenueByEventChart() {
    ...
    if (!window.Chart) {
      window
        .EventSphereLoadChart?.()
        .then(renderRevenueByEventChart)
        .catch(() => {});
      return;
    }
    ...
    if (!byEvent.length) {
      if (empty)
        empty.innerHTML = emptyBlock(
          "bi-pie-chart",
          tr("organizer.no_event_revenue", "No event revenue yet."),
          tr("organizer.no_event_revenue_copy", "Revenue by event will appear after sales."),
        );
      return;
    }
    if (empty) empty.innerHTML = "";
    ...
    window._chartCat = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [
          {
            data: byEvent.map((item) => Number(item.revenue || 0)),
            backgroundColor: ["#5B8CFF", "#8B5CF6", "#22C55E", "#F59E0B", "#EC4899", "#14B8A6"],
          },
        ],
      },
  ...
  function renderTicketAnalytics() {
    renderSalesChart(
      "chartSalesDay",
      state.salesDayTrends,
      "[data-organizer-sales-day-empty]",
      "Sales by day",
    );
    renderSalesChart(
      "chartSalesWeek",
      state.salesWeekTrends,
      "[data-organizer-sales-week-empty]",
      "Sales by week",
    );
    renderTopEvents();
  }
  ...
  function renderSalesChart(canvasId, data, emptySelector, label) {
    ...
    if (!window.Chart) {
      window
        .EventSphereLoadChart?.()
        .then(() => renderSalesChart(canvasId, data, emptySelector, label))
        .catch(() => {});
      return;
    }
    ...
    if (!labels.length) {
      if (empty)
        empty.innerHTML = emptyBlock(
          "bi-bar-chart",
          tr("organizer.no_sales_chart", "No sales chart yet."),
          tr("organizer.no_sales_chart_copy", "Ticket sales charts will appear after paid orders."),
        );
      return;
    }
    if (empty) empty.innerHTML = "";
  ...
  function renderTopEvents() {
    ...
    `,
      ...
      emptyBlock(
        "bi-trophy",
        tr("organizer.no_top_events", "No top events yet."),
        tr(
  ...
  function renderCheckInLogs() {
    ...
    `,
        ...
        .join("") ||
      emptyRow(4, "bi-clock-history", "No validation logs yet", "Scans will appear here.");
  }
  ...
  async function confirmCheckIn() {
    ...
    try {
      const { data } = await api().fetch("/organizer/tickets/check-in", {
        method: "POST",
        body: state.checkInResult.payload,
      });
  ...
  function stopScanner() {
    ...
    if (empty) {
      empty.hidden = false;
      empty.innerHTML =
        '<i class="bi bi-camera-video"></i><span>Start camera scanner or use manual lookup.</span>';
    }
  }
  ...
  function eventPayload() {
    ...
    const limitValue =
      fd.get("max_tickets_per_user") === "custom"
    ...
    return {
      ...
      max_tickets_per_user:
        Number.isInteger(maxTicketsPerUser) && maxTicketsPerUser > 0 ? maxTicketsPerUser : null,
      visibility: "public",
      currency: "USD",
    };
  }
  ...
  function formError(message) {
    ...
    if (el)
      el.innerHTML = message
        ? `<div class="alert border-pro dashboard-note text-danger">${esc(message)}</div>`
        : "";
  }
  ...
  async function saveEvent() {
    ...
    if (
      wantsPublished &&
      !tiers.some((tier) => tier.status === "active" && Number(tier.quantity_total) > 0)
    ) {
    ...
    try {
      ...
      if (eventId) {
        ...
        window.EventSphereNotifications?.add({
          ...
          message: tr("notifications.event_updated_message", "{event} was updated.").replace(
            "{event}",
            savedEvent.title || "Your event",
          ),
        });
        ...
        const { data } = await api().fetch("/organizer/events", {
          method: "POST",
          body: { ...basePayload, status: "draft" },
        });
        savedEvent = data;
        if (!shouldDeferPublish) window.tkToast?.(tr("toast.draft_event_created", "Draft event created."));
      }
      await syncTicketTiers(savedEvent.id, tiers);
      await uploadPendingImages(savedEvent);
      if (shouldDeferPublish) {
        await api().fetch(`/organizer/events/${savedEvent.id}`, {
          method: "PATCH",
        ...
        window.EventSphereNotifications?.add({
          type: "event",
          title: tr("notifications.event_published_title", "Event published"),
          message: tr("notifications.event_published_message", "{event} is now published.").replace(
            "{event}",
            savedEvent.title || "Your event",
          ),
        });
        window.tkToast?.(tr("toast.event_published", "Event published."));
      }
  ...
  async function quickStatus(eventId, status) {
    try {
      ...
      window.EventSphereNotifications?.add({
        ...
        title:
          status === "published"
            ? tr("notifications.event_published_title", "Event published")
            : status === "cancelled"
              ? tr("notifications.event_cancelled_title", "Event cancelled")
              : tr("notifications.event_updated_title", "Event updated"),
        message:
          status === "published"
            ? tr("notifications.event_published_message", "{event} is now published.").replace("{event}", "Your event")
            : status === "cancelled"
              ? tr("notifications.event_cancelled_message", "The event was removed from active discovery.")
              : tr("notifications.event_updated_message", "{event} was updated.").replace("{event}", "Your event"),
      });
      window.tkToast?.(
        status === "published"
          ? tr("toast.event_published", "Event published.")
          : status === "cancelled"
            ? tr("toast.event_cancelled", "Event cancelled.")
            : tr("toast.event_unpublished", "Event unpublished."),
      );
      await refreshEventsAndAnalytics();
    } catch (err) {
    ...
    return $$("[data-tier-row]")
      ...
      .filter(
        (tier) =>
          ...
          tier.price >= 0 &&
          !Number.isNaN(tier.quantity_total) &&
  ...
  async function setEventImageRole(imageId, role) {
    ...
    window.tkToast?.(
      role === "primary"
        ? tr("toast.primary_image_updated", "Primary image updated.")
        : tr("toast.banner_image_updated", "Banner image updated."),
    );
  }
  ...
  function updateImageName(files) {
    ...
    label.textContent = files?.length
      ...
      : tr("organizer.no_images_selected", "No images selected yet.");
  }
  ...
  async function showAttendeeDetails(ticketId) {
    ...
    if (!ticket) return;
    detailModal(
      ticket.attendee?.name || "Attendee details",
      `
      <div class="dashboard-detail-grid">
  ...
  function downloadCsv(type) {
    ...
    const content = [report.headers, ...report.rows]
  ...
  function exportReportPdf(type) {
    ...
    win.document.write(`<!doctype html><html><head><title>${esc(report.title)}</title><style>
      body{font-family:Manrope,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#111827;padding:28px}h1{margin:0 0 4px}p{color:#64748b}table{width:100%;border-collapse:collapse;margin-top:22px}th,td{border-bottom:1px solid #e5e7eb;padding:10px;text-align:left;font-size:12px}th{background:#f8fafc;text-transform:uppercase;color:#475569}.brand{font-weight:700;color:#2563eb}
    </style></head><body><div class="brand">Tiketa</div><h1>${esc(report.title)}</h1><p>Generated ${esc(dateLabel(new Date().toISOString()))}</p><table><thead><tr>${report.headers.map((header) => `<th>${esc(header)}</th>`).join("")}</tr></thead><tbody>${rowsHtml || `<tr><td colspan="${report.headers.length}">No report data available.</td></tr>`}</tbody></table></body></html>`);
    win.document.close();
    win.focus();
    win.print();
  }
  ...
  function bindFilters() {
    ...
    $("[data-organizer-event-search]")?.addEventListener(
      "input",
      debounce(async () => {
        await loadData("events", loadEvents, true);
        renderEvents();
    ...
    $("[data-attendee-search]")?.addEventListener(
      "input",
      debounce(async () => {
        state.attendeePage = 1;
        await loadData("attendees", loadAttendees, true);
  ...
  function bindActions() {
    ...
    $("[data-tier-add]")?.addEventListener("click", () =>
      addTierRow({ name: "New ticket type", price: "0.00", quantity_total: 100, status: "active" }),
    );
    ...
    $("[data-organizer-image-input]")?.addEventListener("change", (event) => {
      ...
      if (files.some((file) => file.size > 5 * 1024 * 1024)) {
        window.tkToast?.(tr("toast.image_size_limit", "Images must be 5 MB or smaller."), "error");
        return;
      }
    ...
    $('[data-organizer-event-form] [name="status"]')?.addEventListener(
      "change",
      syncEventSaveLabel,
    );
    ...
    document.addEventListener("click", async (event) => {
      ...
      if (tierDelete) {
        ...
        if ($$("[data-tier-row]").length <= 1) {
          window.tkToast?.(tr("toast.ticket_type_required", "At least one ticket type is required."), "info");
          return;
        }
        row?.remove();
        return;
      ...
      if (edit) {
        const eventRecord = state.events.find(
          (item) => String(item.id) === String(edit.dataset.eventEdit),
        );
      ...
      if (cancel) {
        if (confirm(tr("confirm.cancel_event", "Cancel this event? Tickets will no longer be valid for check-in."))) {
      ...
      if (report) {
        ...
        if (report.dataset.reportFormat === "pdf") {
          ...
          downloadCsv(type);
          window.tkToast?.(tr("toast.report_csv_exported", "Report exported."));
        }
      }
    });
    ...
    $$("[data-organizer-range]").forEach((button) => {
      button.addEventListener("click", async () => {
        $$("[data-organizer-range]").forEach((btn) =>
  ...
  function setBusy(busy) {
    state.busy = busy;
    $$(
      "[data-event-save], [data-event-edit], [data-event-publish], [data-event-unpublish], [data-event-cancel], [data-organizer-new-event]",
    ).forEach((button) => {
