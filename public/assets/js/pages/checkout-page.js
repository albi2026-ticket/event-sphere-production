  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (els.backLink) {
      els.backLink.href =
        c.source_url ||
        (c.event_slug
          ? window.EventSphereRoutes?.eventUrl?.(c.event_slug) ||
            `/event/${encodeURIComponent(c.event_slug)}`
          : "/events/list");
    }
    ...
    function reservationSecondsRemaining() {
      ...
      return Math.max(
        0,
        Math.floor((new Date(checkoutReservation.expires_at).getTime() - Date.now()) / 1000),
      );
    }
    ...
    function markReservationExpired() {
      ...
      reservationMessage?.replaceChildren(
        document.createTextNode(
          tr("checkout.reservation_expired_message", "Your reservation has expired."),
        ),
      );
      if (els.payBtn) {
    ...
    function setPayReady(ready) {
      ...
      els.payBtn.textContent = ready
        ? `${tr("checkout.pay", "Pay")} ${formattedTotal}`
        : tr("checkout.preparing_checkout", "Preparing checkout...");
    }

    function renderAttendeeFields() {
      if (!els.attendeeFields) return Promise.resolve();

      const attendeeHtml = Array.from(
        { length: quantity },
        (_, index) => `
    ...
    try {
      checkoutReservation = await orders().createCheckoutReservation(
        item.ticket_type_id,
        item.quantity,
      );
      cart().setCart(
      ...
      if (reservationMessage)
        reservationMessage.textContent = "Your tickets are reserved for 5 minutes.";
    ...
    els.payBtn?.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (checkout.checkout_url) location.href = checkout.checkout_url;
          ...
          location.href = `/checkout-success?order_id=${encodeURIComponent(createdOrder.id)}&mock=1`;
      } catch (err) {
        if (createdOrder?.id) {
          try {
    ...
    document.querySelectorAll("[data-alt-pay]").forEach((btn) => {
      btn.addEventListener("click", () => {
        window.tkToast?.(
          `${btn.dataset.altPay} is not enabled yet. Use card mock checkout for testing.`,
          "info",
        );
      });
    });
