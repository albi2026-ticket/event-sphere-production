(function () {
  ...
  function render() {
    ...
    if (state.loading) {
      list.innerHTML =
        window.EventSphereSkeleton?.notificationRows?.(6) ||
        `<div class="dashboard-empty"><span data-i18n="loading.loading_notifications">${window.t?.("loading.loading_notifications") || "Loading notifications..."}</span></div>`;
    ...
    document.addEventListener("click", async (event) => {
      ...
      if (open) {
        ...
        const item = state.notifications.find(
          (notification) => String(notification.id) === String(open.dataset.openNotification),
        );
        try {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    if (
      !auth().requireAuth(["user", "organizer", "owner", "admin"], {
