(function () {
  ...
  function statusBadge(status) {
    ...
    const label = tr(
      keys[status] || "reservation.reservation_pending",
        ...
        <div class="reservation-empty-state">
          ...
          <h3>${esc(
            tr(
              ...
              }[status] || "reservation.no_reservations_found",
              labels[status],
            ),
          )}</h3>
  ...
  function render(loading = false) {
    ...
    statuses.forEach((status) => {
      ...
      root.innerHTML = grouped[status].length
        ? grouped[status].map(reservationCard).join("")
        : emptyState(status);
    });
  }

  function renderDetail(reservation) {
    $("[data-reservation-detail-title]").textContent = tr(
  ...
  async function loadReservations() {
    ...
    reservationsRequest = (async () => {
      ...
      const firstPage = await api().fetch(
        `/my-reservations?per_page=${reservationsPerPage}&page=1`,
      );
          ...
          for (let page = 2; page <= lastPage; page += 1) {
            try {
              const response = await api().fetch(
                `/my-reservations?per_page=${reservationsPerPage}&page=${page}`,
              );
    ...
    document.addEventListener("click", (event) => {
      ...
      if (view) {
        const reservation = reservations.find(
          (item) => Number(item.id) === Number(view.dataset.reservationView),
        );
