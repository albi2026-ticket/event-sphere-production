(function () {
  ...
  function loadRelatedEventsWhenVisible(slug) {
    ...
    const load = () => {
      ...
      eventsApi().getRelatedEvents(slug)
        .then((events) => {
          root.innerHTML = Array.isArray(events) && events.length
            ? events.slice(0, 3).map((event, index) => eventsApi().renderEventCard(event, index)).join('')
