...
(function () {
  ...
  window.EventSphereLoadChart = function () {
    ...
    return window
      .EventSphereLoadScript("https://cdn.jsdelivr.net/npm/chart.js", { crossorigin: "anonymous" })
      .then(() => window.Chart);
  };
  ...
  function readNotifications() {
    return window.EventSphereAuth?.isLoggedIn?.() ? serverNotifications : localNotifications();
  }

  function writeNotifications(items) {
    localStorage.setItem(notificationUserKey(), JSON.stringify(items));
    document.dispatchEvent(
      new CustomEvent("event-sphere:notifications-changed", { detail: { notifications: items } }),
    );
  }
  ...
  async function refreshNotifications(force = false) {
    ...
    notificationRefreshInFlight = Promise.all([
      ...
      .then(([notifications, count]) => {
        serverNotifications = (Array.isArray(notifications.data) ? notifications.data : []).map(
  ...
  async function markNotificationRead(id) {
    if (window.EventSphereAuth?.isLoggedIn?.() && window.EventSphereApi?.fetch) {
      await window.EventSphereApi.fetch(`/notifications/${id}/read`, {
        method: "PATCH",
        body: {},
  ...
  async function markAllNotificationsRead() {
    if (window.EventSphereAuth?.isLoggedIn?.() && window.EventSphereApi?.fetch) {
      await window.EventSphereApi.fetch("/notifications/read-all", {
        method: "PATCH",
        body: {},
        skipAuthRedirect: true,
      });
  ...
  function setupNotifications() {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (item) {
        const notification = readNotifications().find(
  ...
  function startCountdown(el) {
    ...
    tick();
    el._countdownTimer = setInterval(tick, 30000);
  }
  ...
  document.addEventListener("input", (e) => {
    ...
    if (!menu) {
      ...
      menu.style.cssText =
        "position:absolute;z-index:50;left:0;right:0;top:100%;margin-top:8px;padding:.4rem;max-height:280px;overflow:auto";
  ...
  function buildSeatmap(el) {
    ...
    for (let r = 0; r < rows; r++) {
      ...
      row.push(
        `<div class="seat-row"><span class="me-2 text-muted-pro small" style="width:18px">${letter}</span>`,
      );
      for (let c = 0; c < cols; c++) {
        ...
        row.push(
          `<div class="seat ${cls}" data-seat="${letter}${c + 1}" data-price="${price}" title="${letter}${c + 1} — $${price}"></div>`,
        );
      }
      row.push("</div>");
      html.push(row.join(""));
  ...
  function updateSeatSummary() {
    ...
    const list =
      Array.from(sel)
        ...
        .join(", ") || "None";
    const t = document.querySelector("[data-seat-total]");
    if (t) t.textContent = "$" + sum;
    const l = document.querySelector("[data-seat-list]");
    if (l) l.textContent = list;
    const c = document.querySelector("[data-seat-count]");
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      e.target.closest("[data-dash-backdrop]") ||
      (parts.side.classList.contains("open") && e.target.closest(".dash-side a"))
    ) {
  ...
  const skeletonLine = (width = "100%", extra = "") =>
    `<span class="skeleton-line ${extra}" style="width:${html(width)}"></span>`;
  ...
  window.EventSphereSkeleton = {
    ...
    dashboardBlock(rows = 3) {
      return `<div class="dashboard-loading-skeleton">${Array.from({ length: rows })
        .map((_, index) => skeletonLine(index === 0 ? "72%" : index === rows - 1 ? "46%" : "92%"))
        .join("")}</div>`;
    },
    notificationRows(count = 6) {
      return `<div class="notification-skeleton-list">${Array.from({ length: count })
        .map(
          () => `
